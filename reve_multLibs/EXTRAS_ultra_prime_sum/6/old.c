...
int client(int n, int n_copy1, int n_copy2, int n_copy3, int n_copy4, int n_copy5)
{
  int i = 0;
  int sum = 0;
  while (i <= n)
  ...
  return (((((__inlineCall(lib(sum)) == 0) + (__inlineCall(lib(sum_copy1)) == 0)) + (__inlineCall(lib(sum_copy2)) == 0)) + (__inlineCall(lib(sum_copy3)) == 0)) + (__inlineCall(lib(sum_copy4)) == 0)) + (__inlineCall(lib(sum_copy5)) == 0);
}
