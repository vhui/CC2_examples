...
for dirpath, dnames, fnames in os.walk("./"):
  #for dirname in sorted(dnames):
    ...
    """try:
      args0 = shlex.split("ModDiff/cbmc-differential/src/goto-cc/goto-cc %s -o old.out" % old_c_filename)
      proc0 = subprocess.Popen(args0, stdout=PIPE, stderr=PIPE)
      out0, err0 = proc0.communicate(timeout=30)
      args1 = shlex.split("ModDiff/cbmc-differential/src/goto-cc/goto-cc %s -o new.out" % new_c_filename)
      proc1 = subprocess.Popen(args1, stdout=PIPE, stderr=PIPE)
      out1, err1 = proc1.communicate(timeout=30)
      #TODO: WARNING - STATIC PATH ARGUMENT
      args = shlex.split("ModDiff/cbmc-differential/src/goto-diff/goto-diff old.out new.out --semantic-diff --refine")
      proc = subprocess.Popen(args, stdout=PIPE, stderr=PIPE)
      out, err = proc.communicate(timeout=TIMEOUT)
      out_lines = out.decode('utf8').split('\n')
      err_lines = err.decode('utf8').split('\n')
      ...
      #for line in out_lines:
      #    if "uncovered sat" in line:
      #        CC2_SEA_RESULT = NEQ
      #    elif "uncovered unsat" in line:
      #        CC2_SEA_RESULT = EQ
      for line in out_lines:
          if "unchanged TRUE" in line:
              CC2_SEA_RESULT = EQ
    ...
    with open("timingAug3SeqMerge.csv_Reve", 'a') as f:
